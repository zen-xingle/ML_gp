...
import os
from cigp import cigp
# from cigp_v10_w import ConstMeanCIGP
from cigp_w import ConstMeanCIGP
...
def main(data_file):
    ...
    n_train = 50
    n_test = 128
    train_y = data['Ytr'][0].tolist()[0][0:n_train]
    train = data['Ytr'][0].tolist()[0][0:n_train][::4]
    train_yh = data['Ytr'][0].tolist()[2][0:n_train][::4]
    xtr_l = data['xtr'][0:50]
    xtr_h = data['xtr'][0:50][::4]

    eval_y = data['Yte'][0].tolist()[0][0:n_test]
    eval_yh = data['Yte'][0].tolist()[2][0:n_test]
    ...
    ytr = torch.stack([torch.from_numpy(tensorly.tensor_to_vec(train[i])) for i in range(13)], dim = 0)
    ...
    model_LAR.train_adam(ytr, 500, lr = 0.01)
...
if __name__ == '__main__':
    data_file = "data/MultiFidelity_ReadyData/poisson_v4_02.mat"
    main(data_file)