...
def gp_model_block_test(dataset, exp_config):
    ...
    resgp_block = GP_model_block()
    resgp_block.dnm = data_norm_manager
    resgp_block.gp_model = resgp
    ...
    resgp_block.pre_process_block = rho_modules
    resgp_block.post_process_block = rho_modules

    # init optimizer, optimizer is also outsider of the model
    lr_dict = {'kernel': 0.01, 'noise': 0.01, 'others': 0.01, 'rho': 0.}
    params_dict = resgp_block.get_train_params()
    optimizer_dict = [{'params': params_dict[_key], 'lr': lr_dict[_key]} for _key in params_dict.keys()]
    optimizer = torch.optim.Adam(optimizer_dict)
    ...
    for _fi in range(1, fidelity_num):
        ...
        resgp_block.dnm = data_norm_manager
        # resgp_block.gp_model.set_fidelity(_fi-1, _fi, _fi-1, _fi)
        for epoch in range(max_epoch):
            optimizer.zero_grad()
            loss = resgp_block.compute_loss([train_inputs[0], train_outputs[_fi-1]], [train_outputs[_fi]])
            print('epoch {}/{}, loss_nll: {}'.format(epoch+1, max_epoch, loss.item()), end='\r')
            loss.backward()
            optimizer.step()
    ...
    for _fi in range(1, fidelity_num):
        ...
        resgp_block.eval()
        predict_y = resgp_block.predict([eval_inputs[0], predict_y_mean])
