...
def dmfal_test(dataset, exp_config):
    ...
    errorplot_path = os.path.join( 'exp', 'dmfal', data_name, 'errorplot')
    if not os.path.exists(errorplot_path):
        os.makedirs(errorplot_path)
    np.save(os.path.join(errorplot_path, data_name  + '_' + str(dec_rate) + '_' + str(train_inputs[0].shape[0]) + '_result.npy'), (eval_outputs[-1].detach().numpy() - predict_y.detach().numpy()).mean(axis = 0))
...
if __name__ == '__main__':
    ...
    for data_name in data_list:
        for seed in [0]:
            exp_name = os.path.join('exp', 'dmfal', data_name, 'errorplot', 'result.txt')
