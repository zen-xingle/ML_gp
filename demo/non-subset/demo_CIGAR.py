...
def non_subset(first_module, second_module):
    ...
    predict_u, non_subset_var = first_module.predict([non_subset_input])
    merge_var = torch.zeros(s_subset_input.shape[0], device=non_subset_var.device)
    merge_var = torch.cat([merge_var, non_subset_var[:,0]])
    ...
    second_module.base_var = eval_var[:,:1]
    # _, eval_var = first_module.predict([new_input_0])
    second_module.set_up_unknown_yl_var(merge_var, first_module.kstar, first_module.k_l_inv)
...
if __name__ == '__main__':
    ...
    for _dataset in ['poisson_v4_02','burger_v4_02','Heat_mfGent_v5',]:
        for _seed in [0, 1, 2, 3, 4]:
            ...
            ct = controller(CIGP_MODULE, controller_config, ct_module_config, demo_name)
            ct.start_train()

            # second_fidelity_sample = 32
            for subset in [4, 8, 16]:
