...
class cigp(nn.Module):
    def __init__(self, X, Y, normal_y_mode=0):
        ...
        append_info = {'module': str(self), 
                       'module config': {'X_normalize':True, 'Y_normalize': True},
                       'training config': {'lr': 0.1, 'optimizer': 'adam'}}
        self.rc = MLGP_recorder('./how_to_apply_utils_on_your_code/record.txt', append_info, overlap=True)
        self.rc.register(['iter', 'nll'])

        self.torch_cholesky = set_function_as_module_to_catch_error(torch.cholesky)
        os.environ['mlgp_record_file'] = './how_to_apply_utils_on_your_code/record.txt'
    ...
    def forward(self, Xte):
        ...
        Sigma = self.kernel(self.X, self.X) + self.log_beta.exp().pow(-1)

        kx = self.kernel(self.X, Xte)
        # L = torch.cholesky(Sigma) 
        L = self.torch_cholesky(Sigma)  # in this way we can catch the error status
